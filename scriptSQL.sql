-- 1. Crear el usuario (usamos un bloque DO para evitar errores si el usuario ya existe)
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_user WHERE usename = 'user_dawa') THEN
        CREATE USER user_dawa WITH PASSWORD 'user_dawa';
    END IF;
END
$$;

-- 2. Asegurarse de que el esquema exista (por si acaso)
CREATE SCHEMA IF NOT EXISTS dawa;

-- 3. Otorgar permisos de conexión a la base de datos
-- Reemplaza 'nombre_tu_db' por el nombre real de tu base de datos si no es la default
GRANT CONNECT ON DATABASE postgres TO user_dawa;

-- 4. Otorgar permisos sobre el esquema 'dawa'
GRANT USAGE ON SCHEMA dawa TO user_dawa;

-- 5. Otorgar permisos sobre todas las tablas y secuencias actuales en el esquema
...
-- 6. IMPORTANTE: Permisos para tablas que se creen en el futuro
-- Esto evita que tengas que dar permisos cada vez que creas una tabla nueva
ALTER DEFAULT PRIVILEGES IN SCHEMA dawa GRANT ALL ON TABLES TO user_dawa;
ALTER DEFAULT PRIVILEGES IN SCHEMA dawa GRANT ALL ON SEQUENCES TO user_dawa;
